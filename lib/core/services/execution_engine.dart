...
class ExecutionEngine {
  ...
  static String buildRunnerJs(String language, String content) {
    ...
    final escaped = _jsEsc(content);

    switch (lang) {
      case 'javascript':
      case 'js':
        return 'try { window.vsRunner.run("", "", $escaped); }'
            ' catch(e) { window.flutter_inappwebview'
            '?.callHandler("onRunnerLog","error",e.toString()); }';

      case 'html':
        return 'window.vsRunner.runHtml($escaped);';

      case 'css':
        return 'window.vsRunner.run("", $escaped, "");';
      ...
      case 'markdown':
      case 'md':
        // Note: all dollar signs inside the JS string are escaped as \$ in Dart
        return '(function(){'
            ' var md = $escaped;'
            ' var h = md'
            r'   .replace(/^### (.*$)/gim,"<h3>$1</h3>")'
            r'   .replace(/^## (.*$)/gim,"<h2>$1</h2>")'
            r'   .replace(/^# (.*$)/gim,"<h1>$1</h1>")'
            r'   .replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>")'
            r'   .replace(/\*(.+?)\*/g,"<em>$1</em>")'
            '   .replace(/`([^`]+)`/g,"<code style=\\"background:#161B22;padding:2px 6px;border-radius:4px\\">"+"\$1"+"</code>")'
            r'   .replace(/^- (.+)/gm,"<li>$1</li>")'
            r'   .replace(/\n/g,"<br>");'
            ' document.getElementById("vs-user-root").innerHTML ='
            '   "<div style=\\"font-family:system-ui;color:#E6EDF3;background:#0D1117;'
            '   padding:24px;max-width:800px;margin:0 auto;line-height:1.7\\">" + h + "</div>";'
            ' document.body.style.background="#0D1117";'
            '})();';
